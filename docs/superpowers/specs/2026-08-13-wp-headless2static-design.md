...
- トークンは投稿 ID + 有効期限を HMAC 署名したもの。秘密鍵は WP の `AUTH_KEY` を流用し、有効期限は **10 分**。DB 保存不要のステートレス設計。
...
ビルド時に `wp-client` が WP REST API から全コンテンツを取得して静的生成。毎回フルビルドのシンプル設計(数百〜数千記事規模まで想定。それ以上はキャッシュ導入を検討する旨を README に記載)。
...
```bash
pnpm setup      # 初回のみ: wp-env 起動 + シード投入
